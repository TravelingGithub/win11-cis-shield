...
secedit /export /cfg C:\Windows\temp\secpol.cfg
 ...
 #Find-and-Replace to set ResetLockoutCount = 15
 #'\d+' is any integer value
(Get-Content C:\Windows\temp\secpol.cfg) -replace 'ResetLockoutCount = \d+', "ResetLockoutCount = $newValue" | Set-Content C:\Windows\temp\secpol.cfg
